...
TEST_F(LayerZTest, ResponseDims)
{
    ...
    for(int i=0; i<N; i++) {
        ...
        Mat u = signal_.MostRecentMat1f(NAME_OUTPUT_MEM_POT);
        ...
        // check output spikes
        Mat spikes = signal_.MostRecentMat1f(NAME_OUTPUT_SPIKES);
...
TEST_F(LayerZLearnTest, InitialWeights)
{
    ...
    to.IONames(config_);
    to.Response(signal_);
    const Mat1f initial_weights = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS);
...
TEST_F(LayerZLearnTest, BiasDims)
{
    ...
    to.IONames(config_);
    to.Response(signal_);
    const Mat1f initial_bias = signal_.MostRecentMat1f(NAME_OUTPUT_BIAS);
...
TEST_F(LayerZLearnTest, WeightsNotCopied)
{
    ...
    // get from signal
    Mat1f w1 = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS);
    ...
    Mat1f w2 = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS);
    ...
    // downstream computation, this is acceptable
    to_.Response(signal_);
    Mat1f w3 = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS);
...
TEST_F(LayerZLearnTest, Static)
{
    ...
    for(int i=0; i<N; i++) {
        ...
        to_.Activate(signal_);
        to_.Response(signal_);
        Mat w = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS);
...
TEST_F(LayerZLearnTest, Learn_NoFire)
{
    ...
    to_.Response(signal_);
    const Mat1f initial_weights = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS).clone();

    for(int i=0; i<50; i++) {

        signal_.Append(NAME_INPUT_SPIKES, Mat1f::ones(1, nb_afferents_));

        Mat1f bias_prev = signal_.MostRecentMat1f(NAME_OUTPUT_BIAS).clone();

        to_.Activate(signal_);
        to_.Learn();
        to_.Response(signal_);
        Mat1f weights = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS).clone();
        Mat1f bias = signal_.MostRecentMat1f(NAME_OUTPUT_BIAS).clone();
...
TEST_F(LayerZLearnTest, Learn)
{
    ...
    for(int i=0; i<50; i++) {
        ...
        to_.Response(signal_);
        Mat1f bias_prev = signal_.MostRecentMat1f(NAME_OUTPUT_BIAS).clone();
        Mat1f weights_prev = signal_.MostRecentMat1f(NAME_OUTPUT_WEIGHTS).clone();
        ...
        // who spiked?
        Mat spikes_out = signal_.MostRecentMat1f(NAME_OUTPUT_SPIKES);
